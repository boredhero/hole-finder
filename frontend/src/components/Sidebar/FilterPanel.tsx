        ...
        <div className="flex gap-1">
          {BASEMAPS.map(b => (
            <button key={b.value} onClick={() => setBasemap(b.value)}
              className={`flex-1 text-xs py-1.5 rounded transition-colors ${basemap === b.value ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>
              {b.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-1">
          {ALL_TYPES.map(ft => (
            <label key={ft} className="flex items-center gap-2 cursor-pointer text-sm py-0.5">
              ...
              <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: FEATURE_COLORS[ft] }} />
